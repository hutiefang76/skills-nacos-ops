#!/usr/bin/env bash
# nacos-ops skill — Setup (Mac/Linux)
...
echo ""
echo "Usage:"
echo "  $VENV_DIR/bin/python $SKILL_DIR/nacos_config.py list --env local"
echo "  $VENV_DIR/bin/python $SKILL_DIR/nacos_config.py fetch --env local --data-id realtime-job.yaml"
echo "  $VENV_DIR/bin/python $SKILL_DIR/nacos_config.py push --env local --file path/to/nacos.yaml"
echo ""
echo "Demo (本地 docker 起 Nacos):"
echo "  cd <frank-repo>/deploy/test-stack && docker compose up -d nacos"
echo "============================================================"
